...
from timm.models.helpers import named_apply, checkpoint_seq, adapt_input_conv,build_model_with_cfg
...
class VisionTransformer(nn.Module):
    ...
    def get_feature_map(self, x,x_2=None,maxpatch_idxs=None,ratio=None):
        x = self.patch_embed(x)
        if x_2 is not None:
            x_2=self.patch_embed(x_2)    #x_2 bs*196*768
            ratio=ratio.unsqueeze(2).expand(x.shape[0],ratio.shape[1],768).reshape(x.shape[0]*ratio.shape[1],-1)
            mix_x=x.clone()
            mix_x[maxpatch_idxs[0]]=x[maxpatch_idxs[0]]*(1-ratio)+x_2[maxpatch_idxs[1]]*ratio
        return x,x_2,mix_x

    def forward_features(self, x,x_2=None,ratio=None):
        x = self.patch_embed(x)
        if x_2 is not None:
            x_2=self.patch_embed(x_2)    #x_2 bs*196*768
            #saliency score做ratio
            ratio=ratio.unsqueeze(2).expand(x.shape[0],ratio.shape[1],768)
            x=x*ratio+x_2*(1-ratio)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
